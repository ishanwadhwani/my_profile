...
async function sendViaSendGrid({
  ...
  const body = {
    personalizations: [
      { to: [{ email: process.env.CONTACT_TO || "iwadhwani029@gmail.com" }] },
    ],
    from: {
      email: fromEmail || process.env.CONTACT_FROM || "no-reply@example.com",
    },
    subject,
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (process.env.SENDGRID_API_KEY) {
      ...
      return NextResponse.json(
        { message: "Message sent (via SendGrid) — thank you!" },
        { status: 200 }
      );
    } else if (
      ...
      return NextResponse.json(
        { message: "Message sent (via SMTP) — thank you!" },
        { status: 200 }
      );
    } else {
      ...
      return NextResponse.json(
        {
          message:
            "No email backend configured. Please set SENDGRID_API_KEY or SMTP_* env vars. Using mailto fallback is recommended in the UI.",
        },
        { status: 501 }
      );
    }
